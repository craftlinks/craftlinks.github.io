...
const settings = {
  scale:
    (0.85 * Math.min(window.innerHeight, window.innerWidth)) / uniforms.rez,
...
async function main() {
  ...
  document.addEventListener("click", (e) => {
    ...
    gpu.queue.writeBuffer(
      ...
      new Float32Array([mouse.x, mouse.y, 1.0, 1.0])
    );
  ...
  const draw = () => {
    ...
    pass.dispatchWorkgroups(
      Math.ceil(uniforms.preyCount / settings.agentWorkGroups)
    );
    pass.setPipeline(fadePipeline);
    pass.dispatchWorkgroups(
      Math.ceil(uniforms.rez / 16),
      Math.ceil(uniforms.rez / 16)
    );
    ...
    gpu.queue.writeBuffer(
      frameCountBuffer,
      0,
      new Float32Array([uniforms.time++])
    );
