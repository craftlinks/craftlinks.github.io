...
struct Matrix {
  a: array<wrapped_f32,4> // stride 16
}
...
fn reset(@builtin(global_invocation_id) id : vec3u) {
  // let seed = f32(id.x*id.x)/f32(count);
  // var p = vec2(r(seed), r(seed + 0.1));
  let color = u32(floor(r(sin(f32(id.x))) * f32(number_of_colors)));
  // p *= rez;
  // positions[id.x] = p;
...
fn force(r: f32, a:f32) -> f32 {
  ...
  if (r < beta) { return r / beta -1;}
    ...
    return a * (1 - abs(2.0 * r - 1.0 - beta)) / (1.0 - beta);
  }
  else {return 0.0;}
}
...
fn simulate(@builtin(global_invocation_id) id : vec3u) {
  ...
  //     if(l < f32(sizei)) {
  //       // pixels[index(p+vec2(f32(x), f32(y)))] += vec4(f32(colors[id.x])/f32(number_of_colors),0.5,1.0, 1.0) ;
  //       pixels[index(p+vec2(f32(x), f32(y)))] += .9 * ( vec4(hsv2rgb(f32(count)/32.0*f32(colors[id.x])*0.3, 0.9, 1.0), 1.0) * (1.0-l/f32(sizei)));
  //     }
  //   }
  // }

  // Draw
  pixels[index(p)] =vec4(hsv2rgb(f32(count)/36*f32(colors[id.x]), 0.9, 1.0), 1.0);
}
